...
[ExecuteInEditMode]
//添加到菜单选项中
[AddComponentMenu("Blur/RapidBlurEffect")]
...
public class RapidBlurEffect : MonoBehaviour
{
    ...
    #region Variables
        //指定shader名称
    private string ShaderName = "Blur/RapidBlurEffect";
    ...
    //降采样次数
    [Range(0, 6), Tooltip("[降采样次数]向下采样次数。此值越大，则采样间隔越大，需要处理的像素点越少，运行速度越快。")]
    ...
    //迭代次数
    [Range(0, 8), Tooltip("[迭代次数]此值越大，则模糊操作的迭代次数越多，模糊效果越好，大门消耗越大。")]
    ...
    void OnRenderImage(RenderTexture sourceTexture, RenderTexture destTexture)
    {
        ...
        if (CurShader!=null)
        {
            ...
            //拷贝sourceTexure中的选人数据到renderBuffer，并仅绘制指定的pass0的纹理数据
            Graphics.Blit(sourceTexture,renderBuffer,material,0);

            //[2]根据BluriIterations(迭代次数），来进行指定次数的迭代操作
            for (int i = 0; i < BlurIterations; i++)
            {
                ...
                //将tempBuffer赋值给renderBuffer，此时renderBuffer里面pass0和pass1的数据局已经准备好
                renderBuffer = tempBuffer;

                //[2.3]处理shader的通道2，竖直方向模糊处理
                //获取临时渲染纹理
                tempBuffer = RenderTexture.GetTemporary(renderWidth, renderHeight, 0, sourceTexture.format);
                //拷贝renderBuffer中的渲染数据到tempBuffer，并仅绘指定的pass2的纹理数据
                Graphics.Blit(renderBuffer,tempBuffer,CurMaterial,2);

                //[2.4]得到pass0、pass1和pass2的数据都已准备好的renderBuffer
                //再次清空renderBuffer
                RenderTexture.ReleaseTemporary(renderBuffer);
                //再次将tempBuffer赋值给renderBuffer，此时renderbuffer里面pass0，pass1,和pass2的数据都已准备好
                renderBuffer = tempBuffer;
            }
        ...
        //着色器实例为空，直接拷贝屏幕上的效果，此情况下时没有实现屏幕特效的
        else
        {
            //直接拷贝源纹理到目标渲染纹理
